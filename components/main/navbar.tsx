...
export const Navbar = () => {
  ...
  return (
    <div className="w-full h-[75px] fixed top-0 z-50 px-6"
      style={{
        background: "rgba(3,0,20,0.55)",
        backdropFilter: "blur(12px)",
        borderBottom: "1px solid rgba(113,47,255,0.15)",
        boxShadow: "0 2px 20px rgba(42,14,97,0.3), 0 1px 15px 0 rgba(113,47,255,0.3)",
      }}
    >
        ...
        <div className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map((link) => (
            ...
            >
              ...
              <span className="block h-[1.5px] w-0 group-hover:w-full rounded-full transition-all duration-300"
          ...
          {/* Divider */}
          <div className="w-px h-6 mx-2" style={{ background: "rgba(113,47,255,0.25)" }} />
          ...
          {SOCIALS.map(({ link, name, icon: Icon }) => (
            <Link
              ...
              target="_blank"
              rel="noreferrer noopener"
              className="p-2 rounded-lg transition-all duration-200 hover:bg-[rgba(113,47,255,0.12)]"
            >
              <Icon className="h-5 w-5" style={{ color: "rgba(180,160,255,0.6)" }} />
            </Link>
          ))}
        </div>
      </div>
      ...
      {isMobileMenuOpen && (
        ...
        >
          {NAV_LINKS.map((link) => (
            ...
            >
              <svg width="16" height="16" viewBox="0 0 24 24" style={{ fill: "rgba(180,160,255,0.7)" }}>
                {NAV_ICONS[link.title] ?? <circle cx="12" cy="12" r="6" />}
              </svg>
              <span className="text-sm tracking-widest uppercase font-semibold" style={{ color: "rgba(180,160,255,0.7)" }}>
                {link.title}
              </span>
            </Link>
          ))}
          ...
          <div className="flex gap-5 mt-2">
            {SOCIALS.map(({ link, name, icon: Icon }) => (
              <Link key={name} href={link} target="_blank" rel="noreferrer noopener">
                <Icon className="h-6 w-6" style={{ color: "rgba(180,160,255,0.7)" }} />
              </Link>
            ))}
          </div>
        </div>
